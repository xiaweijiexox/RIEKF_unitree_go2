...
class RIEKF:
    ...
    + IMU bias estimation (bg, ba).
    
    State: X = (R, v, p, d_0, ..., d_{Nc-1}, bg, ba)
    Error state dim = 9 + 3*Nc + 6
    Layout: phi=0:3, v=3:6, p=6:9, d_legs=9:9+3*Nc, bg=9+3*Nc:12+3*Nc, ba=12+3*Nc:15+3*Nc
    
    Bias blocks are always at the END of the state vector, after all contact anchors.
    When anchors are added/removed, bias indices shift accordingly.
    """
    ...
    def reset(self):
        ...
        self.active_legs = []
        self.d = {}
        # Base P: [phi(3), v(3), p(3), bg(3), ba(3)] = 15x15
        P_diag = np.concatenate([
            np.ones(9)  * 0.1,    # phi, v, p
            np.ones(3)  * 1e-4,   # bg
            np.ones(3)  * 1e-3,   # ba
        ])
    ...
    @property
    def dim(self):
        return 9 + 3 * len(self.active_legs) + 6  # +6 for bg, ba

    @property
    def _n_d(self):
        return len(self.active_legs)

    @property
    def _bg_start(self):
        return 9 + 3 * self._n_d

    @property
    def _ba_start(self):
        return 9 + 3 * self._n_d + 3
    ...
    def augment(self, leg_idx, h_p_body):
        ...
        self.active_legs.sort()
        
        # Insert 3 rows/cols BEFORE the bias block (bias stays at the end)
    ...
    def _Ad(self):
        ...
        bg_s = self._bg_start
        ba_s = self._ba_start
        Ad[bg_s:bg_s+3, bg_s:bg_s+3] = np.eye(3)
        Ad[ba_s:ba_s+3, ba_s:ba_s+3] = np.eye(3)
        return Ad
    ...
    def _A(self):
        ...
        bg_s = self._bg_start
        ba_s = self._ba_start
        A[3:6, 0:3] = self.skew(self.g_vec)
        A[6:9, 3:6] = np.eye(3)
        A[0:3, bg_s:bg_s+3] = -R
        A[3:6, bg_s:bg_s+3] = -self.skew(self.v) @ R
        A[6:9, bg_s:bg_s+3] = -self.skew(self.p) @ R
        A[3:6, ba_s:ba_s+3] = -R
        for i, leg in enumerate(self.active_legs):
            s = 9 + 3 * i
            A[s:s+3, bg_s:bg_s+3] = -self.skew(self.d[leg]) @ R
        return A
    ...
    def predict(self, omega_tilde, a_tilde):
        ...
        for i in range(self._n_d):
            s = 9 + 3*i
            Cov_w[s:s+3, s:s+3] = self.Sal
        bg_s = self._bg_start
        ba_s = self._ba_start
        Cov_w[bg_s:bg_s+3, bg_s:bg_s+3] = self.Sbg
        Cov_w[ba_s:ba_s+3, ba_s:ba_s+3] = self.Sba
    ...
    def update(self, h_p_body, leg_idx):
        ...
        bg_s = self._bg_start
        ba_s = self._ba_start
        self.bg += delta_xi[bg_s:bg_s+3]
        self.ba += delta_xi[ba_s:ba_s+3]
...
class BatchRIEKF:
    """N个env并行的RIEKF管理器(动态增删锚点 + bias 版)"""
