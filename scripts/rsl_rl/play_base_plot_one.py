...
class RealtimeVelocityPlotter:
    ...
    def __init__(self, window_size: int = 500, dt: float = 0.02):
        ...
        # --- set up figure ---
        plt.ion()
        self.fig, self.axes = plt.subplots(3, 1, figsize=(15, 7), sharex=True)
...
class VelocityMetricEvaluator:
    ...
    def __init__(self, dt: float, start_delay: float = 3.0, duration: float = 10.0,
                 save_path: str = "velocity_metrics_base_one.txt"):
        self.dt = float(dt)
...
class RIEKF:
    ...
    def predict(self, omega_tilde, a_tilde):
        ...
        if np.any(np.isnan(Ad)) or np.any(np.isinf(Ad)) or np.max(np.abs(Ad)) > 1e4:
            print("🚨 发现异常: Ad 矩阵崩溃!")
            print(f"Ad max value: {np.max(np.abs(Ad)):.2e}")
            
            # 👉 新增：抓捕发疯的内部状态变量
            print("🔍 内部状态探头反馈:")
            print(f"   R (旋转矩阵) 最大绝对值: {np.max(np.abs(self.R)):.2e}  <-- 正常应在 1 左右")
            print(f"   v (机体速度) 最大绝对值: {np.max(np.abs(self.v)):.2e}  <-- 正常不应超过几十")
            print(f"   p (机体位置) 最大绝对值: {np.max(np.abs(self.p)):.2e}  <-- 正常不应超过几百")
            print(f"   d (足端位置) 最大绝对值: {np.max(np.abs(self.d)):.2e}  <-- 正常不应超过几百")
            print("-" * 40)
            
        # 检查 Cov_w 矩阵
        if np.any(np.isnan(Cov_w)) or np.any(np.isinf(Cov_w)) or np.max(np.abs(Cov_w)) > 1e4:
            print("🚨 发现异常: Cov_w 矩阵崩溃!")
            print(f"Cov_w max value: {np.max(np.abs(Cov_w)):.2e}")
...
def main():
    ...
    metric_eval = VelocityMetricEvaluator(
        ...
        start_delay=3.0,
        duration=10.0,
        save_path="velocity_metrics_base_one.txt",
    )
