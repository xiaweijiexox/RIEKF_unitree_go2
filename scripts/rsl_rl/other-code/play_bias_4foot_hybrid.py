...
class RealtimeVelocityPlotter:
    """Matplotlib real-time rolling-window plotter for vx/vy/vz CMD vs GT vs EKF (body frame)."""
...
class VelocityMetricEvaluator:
    def __init__(self, dt: float, start_delay: float = 3.0, duration: float = 10.0,
                 save_path: str = "velocity_metrics_4foots_hybrid.txt"):
        self.dt = float(dt)
...
def main():
    ...
    riekf = BatchRIEKF(
        ...
        dt=dt,
        # ---- Hybrid filter 参数(和 play_bias_plot_metric.py 保持一致)----
        sigma_v=0.45,
        sigma_p=0.05,
        pos_meas_noise_xy=0.08,
        pos_meas_noise_z=0.14,
    ...
    metric_eval = VelocityMetricEvaluator(
        ...
        start_delay=3.0,
        duration=10.0,
        save_path="velocity_metrics_4foots_hybrid.txt",
    )
    ...
    while simulation_app.is_running():
        ...
        with torch.inference_mode():
            ...
            for bid, cid in zip(foot_body_ids, foot_contact_ids):
                ...
                # 不做 bool 化 — 交给 hybrid 滤波器内部的接触管理层 _update_contact_mode
                # (双阈值滞回 force_on=18 / force_off=7 + 力衰减降权 + 触地降权)
            ...
            if not riekf_initialized:
                ...
                riekf.set_initial_orientation(R_init)
            ...
            if len(done_ids) > 0:
                ...
                riekf.set_initial_orientation(R_after, done_ids.tolist())
            ...
            # 注意:必须重新从 robot 读最新的 quat,因为 env.step() 已经推进了一帧物理。
            # 之前的 bug 是用顶部缓存的 root_quat_w(step 之前),和 root_lin_vel_w 差一帧,
            # 导致投影到 body frame 时姿态偏一个 dt,GT/EKF 都会偏得很多。
            R_body0 = quats_to_R_batch(
                robot.data.root_quat_w[0:1].cpu().numpy()
            )[0]   # (3,3) env 0 的姿态
            gt_vel_w  = robot.data.root_lin_vel_w[0].cpu().numpy()
            ekf_vel_w = riekf.filters[0].get_velocity_hat()   # hybrid 输出平滑后的速度
            ...
            # terminal log (hybrid 版本:只有 bg,无 ba;bg 由 Mahony 互补滤波器学)
