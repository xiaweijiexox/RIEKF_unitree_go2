...
# 解耦架构 (DRIFT 风格):
#
# 子滤波器 1: AttitudeMEKF — 6 维 [δθ(3), δbg(3)]
#   Predict:  gyro 积分传播姿态, bias 随机游走
#   Update:   加速度计重力方向观测 (有接触支撑时)
#   输出:     bias-corrected R 和 bg
#
# 子滤波器 2: KinematicsEKF — 6 维 [εᵛ(3), εᵖ(3)]
#   和原方案完全一致, 用子滤波器 1 的输出做积分
#   anchor-based contact 逻辑不变
#
# 信息流单向: AttitudeMEKF → KinematicsEKF
# bias 不和 v/p 的 Kalman 增益竞争
# ============================================================
class RIEKF:
    def __init__(
        self,
        ...
        sigma_gravity=0.15,    # gravity direction measurement noise (rad)
        # --- kinematics EKF noise ---
        sigma_v=0.45,
        sigma_p=0.05,
        # --- observation noise ---
    ...
    def reset(self):
        ...
        # P_att: 6x6 for [δθ(3), δbg(3)]
        self.P_att = np.diag([
            0.10, 0.10, 0.10,   # δθ  初始姿态不确定性
            0.01, 0.01, 0.01,   # δbg 初始 bias 不确定性 (保守小)
        ]).astype(np.float64)

        # --- 子滤波器 2: 运动学 EKF ---
        self.v = np.zeros(3, dtype=np.float64)
        self.p = np.zeros(3, dtype=np.float64)
        # P_kin: 6x6 for [εᵛ(3), εᵖ(3)]
        self.P_kin = np.diag([
            0.25, 0.25, 0.25,
            0.50, 0.50, 0.50,
        ]).astype(np.float64)

        # anchor-based contact
    ...
    def reset_kinematics_only(self):
        ...
        # 不重置 P_att 和 bg — 姿态滤波器是独立的、持续运行的
    ...
    # ==================================================================
    #  子滤波器 1: Attitude MEKF  [δθ(3), δbg(3)]
    #
    #  Predict:
    #    R ← R · exp(ω·dt),  ω = ω_tilde - bg
    #    F_att = [[-skew(ω), -I]; [0, 0]]  → Fd = I + F_att·dt
    #    Q_att = diag(σ_gyro² dt², σ_bg² dt²)
    #
    #  Update (gravity observation):
    #    预测重力在 body frame:  g_pred = R^T [0,0,-1]
    #    测量重力在 body frame:  g_meas = -a_b / ||a_b||
    #    残差:  e = g_meas × g_pred  (小角度近似)
    #    H_att = [-skew(g_pred), 0₃ₓ₃]  → 观测 δθ 对重力的影响
    # ==================================================================
    def _att_predict(self, omega_tilde):
        ...
        # 线性化: F_att
        # d/dt δθ = -[ω]× δθ - δbg   → F[0:3,0:3] = -skew(ω), F[0:3,3:6] = -I
        # d/dt δbg = 0                → F[3:6,:] = 0
        F_att = np.zeros((6, 6), dtype=np.float64)
        F_att[0:3, 0:3] = -self.skew(omega)
        F_att[0:3, 3:6] = -np.eye(3, dtype=np.float64)

        Fd = np.eye(6, dtype=np.float64) + F_att * dt

        # 过程噪声
        sg2 = (self.sigma_gyro * dt) ** 2
        sbg2 = (self.sigma_bg * dt) ** 2
        Q_att = np.diag([sg2, sg2, sg2, sbg2, sbg2, sbg2]).astype(np.float64)

        self.P_att = Fd @ self.P_att @ Fd.T + Q_att
        self.P_att = 0.5 * (self.P_att + self.P_att.T) + np.eye(6, dtype=np.float64) * 1e-10

        return omega

    def _att_update_gravity(self, a_b, omega_corr, support_weight_sum):
        """用加速度计的重力方向校正姿态和 gyro bias。"""
        ...
        # 预测的 body-frame 重力方向
        g_pred = self.R.T @ np.array([0.0, 0.0, -1.0], dtype=np.float64)
        g_pred /= max(np.linalg.norm(g_pred), 1e-9)
        # 测量的 body-frame 重力方向
        g_meas = -a_b / a_norm

        # 残差: 叉积给出旋转误差 (小角度)
        residual = np.cross(g_meas, g_pred)
        residual = self._clip_vector(residual, 0.25)

        # H_att: 重力方向对 δθ 的 Jacobian
        # g_pred(δθ) ≈ g_pred + skew(g_pred) · δθ
        # 所以 ∂(g_meas × g_pred)/∂δθ ≈ [g_pred]×  (近似)
        # H = [[g_pred]×, 0₃ₓ₃]
        H = np.zeros((3, 6), dtype=np.float64)
        H[0:3, 0:3] = self.skew(g_pred)

        # 观测噪声
        sg = self.sigma_gravity
        R_meas = np.eye(3, dtype=np.float64) * (sg ** 2)

        # Kalman update
        S = H @ self.P_att @ H.T + R_meas
        try:
            K = self.P_att @ H.T @ np.linalg.inv(S)
        ...
        # 应用修正
        self.R = self.R @ self.exp_so3(dtheta)
        self.bg = self.bg + dbg
        self.bg = np.clip(self.bg, -self.max_bg, self.max_bg)

        I_KH = np.eye(6, dtype=np.float64) - K @ H
        self.P_att = I_KH @ self.P_att @ I_KH.T + K @ R_meas @ K.T
        self.P_att = 0.5 * (self.P_att + self.P_att.T) + np.eye(6, dtype=np.float64) * 1e-10

        return True

    # ==================================================================
    #  子滤波器 2: Kinematics EKF  [εᵛ(3), εᵖ(3)]
    #  和原方案完全一致, 用子滤波器 1 输出的 R 做积分
    # ==================================================================
    def _kin_predict(self, a_b):
        a_b = np.asarray(a_b, dtype=np.float64)
        ...
        self.P_kin = F @ self.P_kin @ F.T + Q
        self.P_kin = 0.5 * (self.P_kin + self.P_kin.T) + np.eye(6, dtype=np.float64) * 1e-9

    def _kin_kalman_update(self, residual, H, Rm):
        S = H @ self.P_kin @ H.T + Rm
        try:
            K = self.P_kin @ H.T @ np.linalg.inv(S)
        except np.linalg.LinAlgError:
            return
        dx = K @ residual

        self.v = self.v + dx[0:3]
        self.p = self.p + dx[3:6]

        I_KH = np.eye(6, dtype=np.float64) - K @ H
        self.P_kin = I_KH @ self.P_kin @ I_KH.T + K @ Rm @ K.T
        self.P_kin = 0.5 * (self.P_kin + self.P_kin.T) + np.eye(6, dtype=np.float64) * 1e-9
    ...
    def _position_update(self, p_leg_meas, weight):
        ...
        self._kin_kalman_update(residual, H, Rm)
    ...
    def step(self, omega_b, a_b, h_body_list, force_norm_list):
        ...
        # 2. 子滤波器 1: 姿态 predict + gravity update
        omega_corr = self._att_predict(omega_b)
        self._att_update_gravity(a_b, omega_corr, support_weight_sum)

        # 3. 子滤波器 2: 运动学 predict (用已校正的 R)
        self._kin_predict(a_b)

        # 4. Anchor-based contact observations (与原方案完全一致)
        p_candidates = []
        p_weights = []
        v_candidates = []
        v_weights = []
