...
class RIEKF:
    ...
    def step(self, omega_b, a_b, h_body_list, force_norm_list):
        # 0. 坐标原点跟随: 把 p 平移到原点, 同步平移所有 anchor
        #    这样 At 中的 (p̄)× 项始终很小, 避免协方差发散
        p_shift = self.p.copy()
        self.p -= p_shift
        for i in range(4):
            if self.anchor_valid[i]:
                self.foot_anchor_w[i] -= p_shift
            if not np.isnan(self.prev_p_leg_meas[i]).any():
                self.prev_p_leg_meas[i] -= p_shift
