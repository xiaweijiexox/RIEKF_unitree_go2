...
class RealtimeVelocityPlotter:
    """Matplotlib real-time rolling-window plotter: CMD vs GT vs EKF + gyro bias (env 0, body frame).

    5 subplots: 3 velocity (CMD/GT/EKF overlay) + 2 bias (bg_xy, bg_z).
    """
    ...
    BIAS_COLORS = ["#e377c2", "#17becf", "#bcbd22"]
...
def main():
    ...
    while simulation_app.is_running():
        ...
        with torch.inference_mode():
            ...
            # step_counter += 1 在 policy 之后,这里用的是"当前这一步"的本地时间
            t_local = step_counter * dt
            if t_local < 8.0:
            ...
            plotter.update(gt_vel, ekf_vel, bg, cmd_fixed)
