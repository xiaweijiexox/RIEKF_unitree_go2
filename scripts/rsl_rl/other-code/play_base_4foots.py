...
class RealtimeVelocityPlotter:
    """Matplotlib real-time rolling-window plotter: CMD vs GT vs EKF (body frame)."""

    LABELS = ["vx", "vy", "vz"]
    COLORS_GT = ["#1f77b4", "#2ca02c", "#d62728"]
...
class VelocityMetricEvaluator:
    def __init__(self, dt: float, start_delay: float = 3.0, duration: float = 10.0,
                 save_path: str = "velocity_metrics_base_4foots.txt"):
        self.dt = float(dt)
...
class RIEKF:
    ...
    def reset(self):
        ...
        # 状态维度 21: [R(3), v(3), p(3), d1(3), d2(3), d3(3), d4(3)]
    ...
    def _Ad(self):
        """21x21 Adjoint for 4-foot RIEKF.
        State layout: [phi, v, p, d0, d1, d2, d3], each 3-d.
        Each anchor d_i has its own R block; cross terms come from skew(d_i)@R.
        """
        R = self.R
        v, p = self.v, self.p
        Z = np.zeros((3, 3))
        ...
        I21[3:6,   0:3] = self.skew(v) @ R
        I21[6:9,   0:3] = self.skew(p) @ R
        for k in range(4):
            I21[9 + 3*k:12 + 3*k, 0:3] = self.skew(self.d[k]) @ R
        return I21

    def _A(self):
        """21x21 continuous-time linearized dynamics.
        Only non-zero blocks: gravity coupling (v <- phi) and position-velocity (p <- v).
        """
        Z, I = np.zeros((3, 3)), np.eye(3)
        A = np.zeros((21, 21))
        A[3:6, 0:3] = self.skew(self.g_vec)   # phi -> v via gravity
        A[6:9, 3:6] = I                        # v   -> p
        # d_k are static in the world frame (anchors), so their rows are all zero
        return A

    def predict(self, omega_tilde, a_tilde):
        self.omega_tilde = omega_tilde  # update 里要用
        """
        预测步
        omega_tilde: 陀螺仪读数, shape (3,)
        a_tilde:     加速度计读数(含 +g,静止时 ≈ [0,0,+9.81]), shape (3,)
        """
        dt = self.dt
        ...
        # 正确的二阶积分:位置必须包含 0.5·a·dt²,否则 z 方向有系统性偏置
        self.R = R @ self.exp_so3(omega_tilde * dt)
        self.v = v + a_w * dt
        self.p = p + v * dt + 0.5 * a_w * dt * dt

        # 21 维过程噪声矩阵:phi, v, p, d0..d3
        Z = np.zeros((3, 3))
        Cov_w = np.zeros((21, 21))
        Cov_w[0:3,   0:3]   = self.Sg      # gyro
        Cov_w[3:6,   3:6]   = self.Sa      # accel (已各向异性)
        ...
        # Ad 正常范围应该 <= 几十;状态位置被 centering 后一般 <1e2
        if np.any(np.isnan(Ad)) or np.any(np.isinf(Ad)) or np.max(np.abs(Ad)) > 1e4:
            ...
            print(f"   v={np.max(np.abs(self.v)):.2e}  p={np.max(np.abs(self.p)):.2e}  "
                  f"d={np.max(np.abs(self.d)):.2e}")

        Q_bar = Ad @ Cov_w @ Ad.T
        F = np.eye(21) + A * dt
        self.P = F @ self.P @ F.T + Q_bar * dt

        # 状态发散检测,自动软重置(只清 v/p/d,保留姿态)
    ...
    def update(self, h_p_body, contact, leg_idx):
        ...
        # H 在 21 维状态上:只碰 p (列 6:9) 和对应的 d_leg (列 9+3*leg_idx : 12+3*leg_idx)
        H_p = np.zeros((3, 21))
        H_p[:, 6:9] = -np.eye(3)
        d_start = 9 + 3 * leg_idx
        ...
        # Joseph 形式协方差更新(数值稳定)
        I_KH = np.eye(21) - K @ H_p
        self.P = I_KH @ self.P @ I_KH.T + K @ N_p @ K.T
        self.P = 0.5 * (self.P + self.P.T)
        self.P += np.eye(21) * 1e-8
...
def main():
    ...
    metric_eval = VelocityMetricEvaluator(
        ...
        start_delay=3.0,
        duration=10.0,
        save_path="velocity_metrics_base_4foots.txt",
    )
...
# --- 新增：EKF 延时启动计数器 ---
    ...
    while simulation_app.is_running():
        ...
        with torch.inference_mode():
            ...
            # 统一坐标系:转 body frame,和 cmd 对齐
            # 注意:用 env.step() 之后的最新 quat,因为 gt velocity 也是 step 之后的
            root_quat_after = robot.data.root_quat_w.cpu().numpy()
            ...
            try:
                ...
                # base_velocity 布局 [vx, vy, yaw_rate] — vz 位置填 0
            ...
            # metric 用 body frame 的 GT vs EKF:评估 EKF 估计质量
