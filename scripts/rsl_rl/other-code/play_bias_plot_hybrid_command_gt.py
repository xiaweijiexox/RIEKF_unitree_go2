...
class RealtimeVelocityPlotter:
    ...
    def __init__(self, window_size: int = 500, dt: float = 0.02):
        ...
        self.fig.suptitle("CMD vs GT vs EKF Velocity + Gyro Bias (env 0, body frame)", fontsize=13)
    ...
    def update(self, gt_vel: np.ndarray, ekf_vel: np.ndarray,
        ...
        if cmd_vel is not None:
            ...
            # keep cmd buffer aligned in length with others
            for i in range(3):
        ...
        for i in range(3):
            ...
            # auto-scale includes cmd so command steps are always visible
...
class VelocityMetricEvaluator:
    ...
    def __init__(self, dt: float, start_delay: float = 3.0, duration: float = 10.0,
                 save_path: str = "velocity_metrics_bias_hybrid.txt"):
        self.dt = float(dt)
...
def main():
    ...
    metric_eval = VelocityMetricEvaluator(
        ...
        start_delay=3.0,
        duration=10.0,
        save_path="velocity_metrics_bias_hybrid.txt",
    )
    ...
    while simulation_app.is_running():
        ...
        with torch.inference_mode():
            ...
            # Transform GT & EKF into body frame so they share the coordinate
            # frame with the velocity command (which is expressed in base frame).
            ...
            plotter.update(gt_vel_b, ekf_vel_b, bg, cmd_vel_b)
            ...
            print(
                ...
                f"GT  vx={gt_vel_b[0]:+.3f} vy={gt_vel_b[1]:+.3f} vz={gt_vel_b[2]:+.3f} | "
