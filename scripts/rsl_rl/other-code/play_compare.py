...
# =====================================================================
# Real-time rolling-window plotter: GT vs EKF velocity + gyro bias (env 0)
# Used for the HYBRID RIEKF (5 subplots: vx/vy/vz + bg_xy + bg_z)
# =====================================================================
class HybridPlotter:
    ...
    def __init__(self, window_size: int = 500, dt: float = 0.02):
        ...
        self.fig.suptitle("HYBRID RIEKF: GT vs EKF Velocity + Gyro Bias (env 0)", fontsize=13)
    ...
    def update(self, gt_vel: np.ndarray, ekf_vel: np.ndarray, bg: np.ndarray = None):
...
# Used for the SIMPLE (baseline) RIEKF (3 subplots: vx/vy/vz only)
# =====================================================================
class SimplePlotter:
    """Matplotlib real-time rolling-window plotter for vx/vy/vz GT vs EKF
    for the simple / baseline RIEKF (no bias estimation).
    """
    ...
    def __init__(self, window_size: int = 500, dt: float = 0.02):
        ...
        self.fig.suptitle("SIMPLE RIEKF (baseline): GT vs EKF Velocity (env 0)", fontsize=13)
...
# =====================================================================
# Dual metric accumulator: evaluates TWO EKFs against the SAME ground truth
# inside a fixed window [start_delay, start_delay + duration].
# Reports metrics side-by-side to one txt file.
# =====================================================================
class DualMetricEvaluator:
    """Accumulates GT and TWO EKF velocity estimates simultaneously.
    Computes scalar metrics for each filter and writes a side-by-side
    comparison report when the window closes.
    """

    def __init__(self, dt: float, start_delay: float = 3.0, duration: float = 10.0,
                 save_path: str = "velocity_metrics.txt",
                 name_a: str = "HYBRID", name_b: str = "SIMPLE"):
        self.dt = float(dt)
        ...
        self.gt_list = []
        self.ekf_a_list = []   # hybrid
        self.ekf_b_list = []   # simple
        self.t_list = []
    ...
    def reset(self):
        ...
        self.gt_list.clear()
        self.ekf_a_list.clear()
        self.ekf_b_list.clear()
        self.t_list.clear()
        self.step_count = 0
        self.finalized = False
        print("[METRIC] Dual evaluator reset.")

    def update(self, gt_vel: np.ndarray, ekf_a_vel: np.ndarray, ekf_b_vel: np.ndarray):
        """Push one sample of (GT, filter_A, filter_B). Only records inside window."""
        if self.finalized:
            return
        ...
        if t < self.start_delay + self.duration:
            ...
            self.ekf_a_list.append(np.asarray(ekf_a_vel, dtype=np.float64).copy())
            self.ekf_b_list.append(np.asarray(ekf_b_vel, dtype=np.float64).copy())
    ...
    @staticmethod
    def _compute_metrics(gt: np.ndarray, ekf: np.ndarray):
        """Return a dict of scalar metrics for one filter."""
        err = ekf - gt
        ...
        return {
            ...
            "mae_xyz": mae_xyz,
            "max_err_xyz": max_err_xyz,
            "std_err_xyz": std_err_xyz,
            "rel_rmse_xyz": rel_rmse_xyz,
        }
    ...
    def finalize(self):
        ...
        ekf_a = np.stack(self.ekf_a_list, axis=0)
        ekf_b = np.stack(self.ekf_b_list, axis=0)
        N = gt.shape[0]

        ma = self._compute_metrics(gt, ekf_a)
        mb = self._compute_metrics(gt, ekf_b)

        # Build side-by-side report
        def row(label, a_vals, b_vals, fmt="{:8.4f}"):
            a_str = "  ".join(fmt.format(v) for v in a_vals)
            b_str = "  ".join(fmt.format(v) for v in b_vals)
            return f"{label:<18s} | {a_str}  |  {b_str}\n"

        header = (
            f"=============================================================================\n"
            f"               VELOCITY METRICS: {self.name_a} vs {self.name_b}\n"
            f"=============================================================================\n"
            f"Window: [{self.start_delay:.1f}s, {self.start_delay + self.duration:.1f}s]"
            f"  |  samples: {N}  |  dt: {self.dt:.4f}s\n"
            f"-----------------------------------------------------------------------------\n"
            f"{'':<18s} | {self.name_a:^30s}  |  {self.name_b:^30s}\n"
            f"{'':<18s} |     vx        vy        vz     |      vx        vy        vz\n"
            f"-----------------------------------------------------------------------------\n"
        )
        body = (
            row("RMSE      (m/s)",  ma["rmse_xyz"],     mb["rmse_xyz"])
            + row("MAE       (m/s)",  ma["mae_xyz"],      mb["mae_xyz"])
            + row("Max |err| (m/s)",  ma["max_err_xyz"],  mb["max_err_xyz"])
            + row("Err std   (m/s)",  ma["std_err_xyz"],  mb["std_err_xyz"])
            + row("Rel RMSE  (-)  ",  ma["rel_rmse_xyz"], mb["rel_rmse_xyz"])
        )
        footer = (
            f"-----------------------------------------------------------------------------\n"
            f"Total 3D RMSE (m/s): {self.name_a} = {ma['rmse_total']:.4f}   |   "
            f"{self.name_b} = {mb['rmse_total']:.4f}\n"
            f"=============================================================================\n"
        )
        ...
        try:
            with open(self.save_path, "w") as f:
                f.write(msg)
            print(f"[METRIC] >>> window closed, saved comparison to {self.save_path} <<<")
        except Exception as e:
            print(f"[METRIC] Failed to save metrics: {e}")


# =====================================================================
# (Legacy) single-filter metric evaluator, kept for reference but unused.
# =====================================================================
...
# ============================================================
# Baseline RIEKF (simple 12-dim R/v/p/d) — renamed to avoid clash
# with the main hybrid filter above. Class body is identical to the
# implementation in play_base_plot.py.
# ============================================================
class SimpleRIEKF:
    """
    右不变扩展卡尔曼滤波器，用于Go2真机/仿真状态估计 (simple baseline)
    ...
    def exp_so3(phi):
        angle = np.linalg.norm(phi)
        if angle < 1e-8:
            return np.eye(3) + SimpleRIEKF.skew(phi)
        K = SimpleRIEKF.skew(phi / angle)
    ...
    def update(self, h_p_body, contact):
        ...
        # State centering: keep p/d near origin to avoid float blow-up
        shift = self.p.copy()
        self.p -= shift
        self.d -= shift
...
class BatchSimpleRIEKF:
    """N 个 env 并行的 simple baseline RIEKF 管理器"""
    def __init__(self, num_envs, dt, **kwargs):
        self.num_envs = num_envs
        self.filters  = [SimpleRIEKF(dt, **kwargs) for _ in range(num_envs)]
...
def main():
    ...
    # =====================================================================
    # Initialize SIMPLE (baseline) RIEKF in parallel — runs on the SAME
    # sensor stream as the hybrid filter, but does NOT feed the policy.
    # =====================================================================
    riekf_simple = BatchSimpleRIEKF(
        num_envs    = raw_env.num_envs,
        dt          = dt,
        sigma_g     = 0.01,
        sigma_a     = 0.1,
    ...
    print(f"[INFO] Simple baseline RIEKF initialized in parallel.")

    # Baseline filter only uses ONE foot (FL). Record its index for later.
    simple_foot_name       = "FL_foot"
    simple_foot_body_id    = robot.body_names.index(simple_foot_name)
    simple_foot_contact_id = cf_sensor.body_names.index(simple_foot_name)

    # =====================================================================
    # Initialize real-time plotters (one per filter) + dual metric evaluator
    # =====================================================================
    plotter_hybrid = HybridPlotter(window_size=args_cli.plot_window, dt=dt)
    plotter_simple = SimplePlotter(window_size=args_cli.plot_window, dt=dt)
    print(f"[INFO] Two real-time plotters initialized (window={args_cli.plot_window} steps)")

    metric_eval = DualMetricEvaluator(
        dt=dt,
        start_delay=3.0,
        duration=10.0,
        save_path="velocity_metrics_compare.txt",
        name_a="HYBRID",
        name_b="SIMPLE",
    )
    print(f"[INFO] Dual metric evaluator initialized "
    ...
    while simulation_app.is_running():
        ...
        with torch.inference_mode():
            ...
            if imu_ok:
                # --- Hybrid filter (main method, feeds policy) ---
                riekf.step(omega_tilde, a_tilde, h_p_body_list, force_norm_list)

                # --- Simple baseline filter (passive, same sensor stream) ---
                # It only consumes FL foot kinematics + a bool contact, so we
                # pick leg 0 (FL) from the four-foot lists and build the right
                # inputs. Sensor measurements (omega/a) are IDENTICAL.
                simple_h_p_body = h_p_body_list[0]  # (N, 3) — FL foot in body
                simple_contact = force_norm_list[0] > 1.0  # (N,) bool — FL contact
                riekf_simple.step(omega_tilde, a_tilde, simple_h_p_body, simple_contact)

            # overwrite policy observations — always from HYBRID filter,
            # so the robot experiences the same action sequence for both filters.
            ...
            # IMPORTANT: reset BOTH filters on episode reset / fall
            ...
            if len(done_ids) > 0:
                ...
                riekf_simple.reset(done_ids.tolist())
                print(f"[IMU_EKF] reset BOTH filters for env_ids={done_ids.tolist()}")
                # Reset plotters and metric evaluator if env 0 was reset
                if 0 in done_ids:
                    plotter_hybrid.reset()
                    plotter_simple.reset()
                    metric_eval.reset()

            # ---- Real-time plot + metric update (env 0 only) ----
            gt_vel = robot.data.root_lin_vel_w[0].cpu().numpy()
            ekf_vel_hybrid = riekf.filters[0].get_velocity_hat()
            ekf_vel_simple = riekf_simple.filters[0].v.copy()
            bg = riekf.filters[0].bg.copy()

            plotter_hybrid.update(gt_vel, ekf_vel_hybrid, bg)
            plotter_simple.update(gt_vel, ekf_vel_simple)
            metric_eval.update(gt_vel, ekf_vel_hybrid, ekf_vel_simple)
            ...
            print(
                ...
                f"HYB vx={ekf_vel_hybrid[0]:+.3f} vy={ekf_vel_hybrid[1]:+.3f} vz={ekf_vel_hybrid[2]:+.3f} | "
                f"SIM vx={ekf_vel_simple[0]:+.3f} vy={ekf_vel_simple[1]:+.3f} vz={ekf_vel_simple[2]:+.3f}"
            )
    ...
    # Clean up: finalize metrics (in case window still open) and close plotters
    metric_eval.finalize()
    plotter_hybrid.close()
