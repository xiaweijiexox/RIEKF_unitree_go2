...
from isaaclab.sensors import ContactSensorCfg, ImuCfg, RayCasterCfg, patterns  # ← 新增 ImuCfg
...
class RobotSceneCfg(InteractiveSceneCfg):
    ...
    robot: ArticulationCfg = ROBOT_CFG.replace(
        ...
        init_state=ArticulationCfg.InitialStateCfg(
            pos=(0.0, 0.0, 0.3),  # 依然保持 0.8 米的掉落高度
            rot=(1.0, 0.0, 0.0, 0.0),
            joint_pos=ROBOT_CFG.init_state.joint_pos,  # <--- 关键修复！继承狗原本的站立关节角度
        )
    )
    ...
    # ← 新增：IMU sensor，挂载在 base body 上
    # gravity_bias 默认 (0, 0, 9.81)，静止时 lin_acc_b 输出 [0, 0, +9.81]，与真实 IMU 行为一致
...
class RobotEnvCfg(ManagerBasedRLEnvCfg):
    ...
    def __post_init__(self):
        ...
        self.scene.imu_sensor.update_period = self.sim.dt  # ← 新增：IMU 与 contact_forces 同频更新
